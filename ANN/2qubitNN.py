...
class HybridFunction(Function):  
    ...
    def backward(ctx, grad_output):
        ...
        loss_second = 0
        for i in range(4):
            loss_first -= (np.log(expectation_left_first) * expectation_right_first)
            loss_second -= (np.log(expectation_left_second) * expectation_right_second)
        ...
        gradient_first = gradient_first * gradient_first
        gradient_second = gradient_second * gradient_second
        gradient_first = gradient_first.numpy()
        gradient_second = gradient_second.numpy()

        
        gradients.append((gradient_first).sum())
        gradients.append((gradient_second).sum())
        
        
        gradients = np.array([gradients]).T
        grad_output = torch.sum(grad_output * grad_output)
        return grad_output * torch.tensor([gradients]).view(-1,2).float(), None, None
